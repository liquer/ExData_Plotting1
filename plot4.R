...
while(!(findstart & findend)){
  ...
  data2 <- data.frame(data1$Date[flag], data1$Time[flag], data1$Global_active_power[flag], data1$Global_reactive_power[flag], data1$Voltage[flag], data1$Sub_metering_1[flag], data1$Sub_metering_2[flag], data1$Sub_metering_3[flag])
  if(sum(flag) != 0){
    if(length(data) == 0){
      data <- data2
    }
...
names(data) <- c("Date", "Time", "Global_active_power", "Global_reactive_power", "Voltage", "Sub_metering_1", "Sub_metering_2", "Sub_metering_3")
...
## Draw the topright
plot(wday, data$Voltage, type = "l", xlab = "datetime", ylab = "Voltage")

## Draw the bottomright
plot(wday, data$Global_reactive_power, type = "l", xlab = "datetime", ylab = "Global_reactive_power")
